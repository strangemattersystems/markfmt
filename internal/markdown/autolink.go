...
// (CM 594 to 610).
func autolinkEnd(src []byte, i, end uint32) uint32 {
	for _, e := range [...]uint32{uriEnd(src, i+1, end), emailEnd(src, i+1, end)} {
		if e > 0 && e < end && src[e] == '>' {
			return e + 1
		}
	}
...
// and end with a letter or digit, separated by '.'.
func emailEnd(src []byte, i, end uint32) uint32 {
	j := i
	for j < end && (isASCIIAlphanumeric(src[j]) || strings.IndexByte(".!#$%&'*+/=?^_`{|}~-", src[j]) >= 0) {
		j++
	}
	if j == i || j == end || src[j] != '@' {
		return 0
