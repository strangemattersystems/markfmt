...
func TestGoldmarkDiffers(t *testing.T) {
	...
	tests := []struct {
		...
		{"skips a form feed in a tag over three lines", "<A\nA\nA\f>", "goldmark deviates, spec section 6.6: FF is whitespace in an HTML tag, as cmark reads it"},
...
var goldmarkDeviations = []struct {
	...
	{"goldmark deviates, spec section 6.6: FF is whitespace in an HTML tag, as cmark reads it", func(t *Tree) bool {
		...
		for i := range src {
			...
			// Each run of whitespace in a tag holds at most one line ending.
			lines := 0
		scan:
